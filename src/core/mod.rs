...
struct BrainFuck<'a, const N: usize>{
    cell_list: &'a mut [u8; N],
    ptr: usize,
...
impl<'a, const N: usize> BrainFuck<'a, N>{
                ...
                '[' => {
                    if let Some(raw_code) = raw_code.get(i..){
                        match find_bracket(raw_code) {
                            Ok(code_part) => token_list.push(Token::Group(self.tokenize(code_part))),
                            Err(e) => panic!("{:?}", e),
                        }
                    }
    ...
    fn interprete(&mut self, token_list: &Vec<Token>){
